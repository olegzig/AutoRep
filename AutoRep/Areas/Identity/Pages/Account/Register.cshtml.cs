...
namespace AutoRep.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var user = new SUser { UserName = Input.Email, Email = Input.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    //    $"Подтвердите вашу почту <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>нажатием сюда</a>.");
                    EmailService emailService = new EmailService();
                    await emailService.SendEmailAsync(
                           Input.Email, "Подтверждение почты",
                        $"Подтвердите вашу почту <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>нажатием сюда</a>.");
